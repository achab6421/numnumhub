...
// 獲取餐廳已有標籤
$allRestaurantTags = getRestaurantTags($id);
$restaurantTags = [];

// 過濾標籤，只保留屬於當前使用者的標籤
foreach ($allRestaurantTags as $tag) {
    foreach ($tagOptions as $userTag) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        if ($result['success']) {
            // 更新餐廳標籤，傳入使用者ID以確保只使用該使用者的標籤
            updateRestaurantTags($id, $tagIds, $userId);
