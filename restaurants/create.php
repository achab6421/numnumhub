...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        if ($result['success']) {
            // 如果有選擇標籤，則關聯標籤
            if (!empty($tagIds) && isset($result['id'])) {
                updateRestaurantTags($result['id'], $tagIds);
            }
                ...
                <form id="restaurantForm" action="<?php echo url('create-restaurant'); ?>" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <input type="text" class="form-control" id="address" name="address" placeholder="例如：雲林縣 虎尾 文化路 64號">
                        ...
                        <small class="form-text text-muted" id="geocodeStatus">請使用格式「縣市 區域 路名 門牌號」以提高準確度</small>
                    </div>
                    ...
                    <!-- 地圖預覽區域 -->
                    <div class="form-group">
                        <div id="mapPreview" style="height: 300px; display: none;" class="mb-3 border rounded"></div>
                    </div>
        ...
        if (address && (!lat || !lng)) {
            ...
            setTimeout(() => {
                ...
                if (latitudeInput.value && longitudeInput.value) {
                    ...
                    Swal.fire({
                        title: '警告',
                        text: '無法獲取地址經緯度，是否繼續保存餐廳？',
                        icon: 'warning',
                        showCancelButton: true,
                        confirmButtonText: '是，繼續保存',
